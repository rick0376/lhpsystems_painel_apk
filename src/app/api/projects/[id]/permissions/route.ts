...
const createPermissionSchema = z.object({
    name: z.string().trim().min(2, "Informe o nome da permissão."),
    key: z
        .string()
        .trim()
        ...
        .regex(
            /^[a-z0-9_]+$/,
            "A chave deve usar apenas letras minúsculas, números e underline.",
        ),
...
export async function POST(request: Request, { params }: RouteProps) {
    ...
    try {
        ...
        if (existing) {
            return NextResponse.json(
                { error: "Este projeto já possui uma permissão com essa chave." },
                { status: 400 },
            );
        }
        ...
        return NextResponse.json(
            { error: "Não foi possível criar a permissão." },
            { status: 500 },
        );
    }
}